...
// colors sampled from the card: sunset, sky, pool, tiles, foliage,
// poker chips, cream card stock, and the two buttons
const BASE_PIECE_DEFS: PieceDef[] = [
  ...
  {kind: 'sparkle', color: '#f8e27a', size: 52},
  {kind: 'sparkle', color: '#ffffff', size: 44},
  {kind: 'sparkle', color: '#7fd4e8', size: 48},
...
// population scales with viewport area: ~22 pieces on a phone,
// ~78 on a laptop, capped at 144 on large displays
const makePieceDefs = (): PieceDef[] => {
  const area = window.innerWidth * window.innerHeight
  const count = Math.min(144, Math.max(20, Math.round(area / 15000)))
